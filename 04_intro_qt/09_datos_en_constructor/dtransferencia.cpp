...
DTranferencia::DTranferencia(QStringList  listacuenta , QWidget *parent , Qt::WindowFlags f): 
QDialog(parent, f){
	...
	layoutDer->addWidget(boto2);
	 ...
	 connect(linea1,SIGNAL(textChanged(const QString & )),
 	 ...
 	 connect(boto1,SIGNAL(clicked()),
 		this, SLOT(slotBotonAceptar()));
 		
 	 connect(boto2,SIGNAL(clicked()),
 		this, SLOT(reject())); 		
...
void DTranferencia::slotBotonAceptar(){
