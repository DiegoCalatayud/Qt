...
void VentanaPrincipal::slotDialogoBuscar(){
	...
	if( dialogoBuscar== NULL ){ 
		dialogoBuscar = new DBuscar();
		
		 connect(dialogoBuscar,SIGNAL(senyalBuscar(QString)),
	 				this,SLOT(slotBuscar(QString)));
	}
...
void VentanaPrincipal::slotBuscar(QString e ){

	etiqueta->setText(e);
	
	QTextDocument* textDocument = editorCentral->document();
